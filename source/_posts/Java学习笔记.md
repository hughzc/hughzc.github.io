...
​    类是在其任何static成员被访问时被加载的，构造器也是static的，只是没有显式写出。初次使用之处也是static初始化发生之处。所有的static对象和static代码块都会在加载时依程序中的顺序（即定义类时的书写顺序）而依次初始化。定义为static只会被初始化一次。

构造器如果是static为什么会被多次初始化？
...
​    同步代码块对于锁的操作是隐式的。后期把锁封装成对象，获取锁与释放锁是锁对象最清楚，变成显示的。现在有lock()与unlock()来对锁进行操作。因此lock对synchronized进行了替代，代码进行改写。
